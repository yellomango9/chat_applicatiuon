    ...
    ...commonChatAggregation(),
    {
      $addFields: {
        ...
        unreadCount: {
          $let: {
            vars: {
              userIdStr: { $toString: currentUserId },
...
// get default group
const getDefaultGroup = (): Promise<any> => {
  return ChatModel.findOne({ isDefault: true }).populate('participants', 'username email avatarUrl');
};

// check if user is in default group
const isUserInDefaultGroup = (userId: Types.ObjectId): Promise<any> => {
